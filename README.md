...
- 💳 Pagamentos: integração com Clerk e webhooks.
...
- Para crédito avulso (fora da assinatura), mapeie os `Clerk Price IDs` para a quantidade de créditos em `src/lib/clerk/credit-packs.ts`.
  - Exemplo:
    ```ts
...
- Configure variáveis de `.env.example` (Clerk, `DATABASE_URL`, etc.)
- Use runtime Node (não Edge) para endpoints com Prisma
- Aponte webhooks (Clerk) para rotas em `src/app/api/webhooks/*`
...
2) Configure variáveis de ambiente (Clerk, `DATABASE_URL`)
3) Build & Runtime:
...
6) Webhooks: aponte Clerk para `src/app/api/webhooks/*` e defina `CLERK_WEBHOOK_SECRET`
